    ...
    <tbody>
        ...
        if(is_array($data['arrPagos'])&&!empty($data['arrPagos'])){
            ...
            foreach($data['arrPagos'] AS $crud){
                $MontoPagado = $MontoPagado + $crud['MontoPagado']; ?>
                <tr>
            ...
            <?php } ?>
            <tr>
                <td class="text-end" colspan="3"><strong>Total Pagado</strong></td>
                <td class="text-end"><?php echo $data['Fnc_DataNumbers']->Valores($MontoPagado, 2); ?></td>
                <?php if(isset($data['rowData']['idEstadoPago'])&&$data['rowData']['idEstadoPago']==1){ ?>
                    <td></td>
                <?php } ?>
            </tr>
